...
def gpt_evaluate(article_text: str, selected_sources: List[str]):
    ...
    try:
        completion = client.chat.completions.create(
            ...
            temperature=0  # 더 결정적인 결과를 위해 온도를 0으로 설정
        )
...
async def analyze_news(request: NewsRequest):
    ...
    # 첫 번째 메인 카테고리만 사용
    if not user_main_categories:
        raise HTTPException(status_code=400, detail="카테고리를 하나 이상 선택해야 합니다.")
    
    main_cat = user_main_categories[0]

    if main_cat not in CATEGORIES:
        raise HTTPException(status_code=404, detail=f"'{main_cat}' 카테고리를 찾을 수 없습니다.")

    # 해당 메인 카테고리의 첫 번째 서브 카테고리만 사용
    if not CATEGORIES[main_cat]:
        raise HTTPException(status_code=404, detail=f"'{main_cat}'에 서브 카테고리가 없습니다.")

    sub_cat = CATEGORIES[main_cat][0]
    
    print(f"단일 분석 모드: [{main_cat}] > [{sub_cat}]의 첫 번째 기사만 분석합니다.")
    
    try:
        fetched_articles = fetch_news_from_google(sub_cat)
        
        if not fetched_articles:
            raise HTTPException(status_code=404, detail=f"'{sub_cat}' 카테고리에서 뉴스를 찾을 수 없습니다.")
        ...
        print(f"단일 기사 처리 중 오류: {e}")
        raise HTTPException(status_code=500, detail=f"기사 처리 중 오류 발생: {e}")
